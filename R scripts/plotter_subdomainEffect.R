res <- read.csv("./Stage 3A ENS/DNA_Religation/projects/RCLPolymer_TwoDomains/results/2018_05_24_12_08_proba-v_Nlr_TADsize.csv")
...
# PROBA POINTS
{p <- ggplot(data = res, aes(x = Nlr, y = repair_probability)) + geom_line()  + geom_ribbon(data = res, aes(x = Nlr, 
                                                                                                           ymin = repair_probability - repair_CIhalflength,
                                                                                                           ymax = repair_probability + repair_CIhalflength), alpha = 0.2) + theme_bw()
p}

# PROBA RASTER
{q <- ggplot(data = res, aes(x = Nc, y = numMonomers)) + geom_raster(aes(fill = repair_probability), interpolate = F)
q + scale_fill_gradientn(colours= heat.colors(100))
}

#MFET RASTER
{k <- ggplot(data = res, aes(x = Nc, y = numMonomers)) + geom_raster(aes(fill = meanFET), interpolate = T)
  k + scale_fill_gradientn(colours= heat.colors(100))
}
