...
\section{Motivation}
Double Strand Breaks (DSB) are a highly dangerous type of DNA damage that occur when both strands of duplex DNA are broken. 
...
\end{equation}
 ...
 In particular, we are interested on the probability of repair after two DSBs. One DSB will be randomly induced in neighbor monomers $A_1$ and $A_2 = A_1 + 1$ (DSB "A"), where $A_1 \sim \mathcal{U}[1,N-g-2]$. the other DSB will be induced in neighbor monomers $B_1$ and $B_2$ (DSB "B"). A deterministic inter-break genomic distance $g$ will be imposed between A and B, defined as the shortest distance between the DSBs along the backbone, i.e. $g = B1 - A2$.
