...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../DNA_Religation/projects/RCLPolymer_CLs@DamageFoci/figures/proba_vs_Nc_g20_Nd2.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../DNA_Religation/projects/RCLPolymer_CLs@DamageFoci/figures/proba_vs_Nc_g4_Nd2.png}
...
\subparagraph*{Summary of the effects of $g$ and $N_c$ on the repair}
...
However \textbf{the repair probability function considering excluded volume interactions is always below the repair probability in the non-excluded case} (Figs. \ref{fig:proba_vs_g_exclusion} and \ref{fig:proba_vs_Nc_exclusion})
...
Now, smaller encounter radius will be considered now to see if excluded volume could help the correct repairs.

Figure \ref{fig:noExclusion_002epsilon} shows the results with and without volume exclusion and with a encounter distance of $\varepsilon = b \cdot 10^{-1} = 0.02 \mu$m. We observe that keeping the same $\varepsilon$ does not provide interesting results. This results motivated the addition of a $\xi$-adaptive $\varepsilon$.
...
\subsubsection{Joint effect of volume exclusion and CL removal}
...
We'll now consider a polymer folded in two connected sub-domains (simulating topologically associating domains (TADs) in DNA) as represented in Fig. \ref{fig:TADSmodel}). We'll define $M$ sub-domains where each sub-domain $i$ has a connectivity $\xi_i$ defined by their number of inner CLs $N_c^{(i)}$. A number $N_{lr}^{(i,j)}$ of large-range CLs connecting different pairs $i \neq j$  of sub-domains are also added. Thus, we define the connectivity matrix $\mathbb{N}_c$ characterizing the TAD-folded RCL polymer by Eq. \ref{eqn: Nc_TAD}.
...
\begin{figure}[!h]
	...
	\caption{Model of the RCL polymer folded in two connected subdomains.}
	\label{fig:TADSmodel}
\end{figure}
