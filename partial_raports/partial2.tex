...
\section{Introduction}
 The model simulated is described in figure \ref{fig:model}. We supposed a random cross-linked polymer (RCL polymer) subdued to Langevin dynamics, i.e., if $(R_t)_{t\geq0} = (R_1, ..., R_N)_{t\geq0}$ are the monomer positions of the polymer in time, the dynamics are described by the equation:
 ...
 $$
 \phi(R) = \frac{\kappa}{2} \sum_{n = 1}^{N-1} ||R_{n+1} - R_{n}||^2 + \frac{\kappa'}{2} \sum_{i,j \in \mathcal{R}} ||R_{i} - R_{j}||^2 =\frac{\kappa}{2} {}^{t}RMR + \frac{\kappa'}{2} {}^{t}RBR $$
...
The results of figure \ref{fig:hists} indicate that the removal of the cross-links seems not affect the mean first encounter time, nor the ratio of fail/repair events. The estimated rate parameter ($\lambda$) is 0.08 for both cases. It should be noted that since the maximum time of simulation is set at 60 seconds (i.e. if nothing happens in 50 seconds the simulation is discarded) the distribution is biased and corresponds to $\mathcal{L}(FET|FET<50)$. For instance, when the simulation maximum time has been increased to 200 seconds, the parameter is estimated at 0.04.
...
We see that regardless the removal of cross-links the repair probability increases in the same way as the inter-break genomic distance increases. So, removing the cross-links in the disconnected monomers does not seem to induce a better repair probability
...
\subsection{Effect of volume exclusion with more restrictive encounter distances}
...
\begin{figure}[h]
	...
	\caption{Control: Probability and 95\% confidence interval of repair, in function of the interbreak distance. 5 RCLs over 100 monomers.}
	\label{fig:1domain}
\end{figure}
...
\item For two DSBs in a TAD: Simulate the repair probability vs. inter-break genomic distance (almost at the limit of the TAD). Does the probability decreases? Test with different number of random connectors (to self: simulate this first, could be interesting) (cf. figure with 2 domains and 10 random connectors per domain, the probability seems to decrease at the end, verify!)
\end{itemize}
