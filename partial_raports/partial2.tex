...
An schematic illustration of an RCL polymer is described in Fig. \ref{fig:model}-A along with its respective Laplacian matrices (Fig.\ref{fig:model}-B). 
 ...
 \subsection{Langevin dynamics of the RCL polymer}
  We supposed the RCL polymer subdued to Langevin dynamics, i.e., if monomer positions of the polymer in time are represented by $(R_t)_{t\geq0} = (R_1, ..., R_N)_{t\geq0}$ with each $R_i \in \mathbb{R}^3, i = 1,...,N$, the dynamics are described by Eq. \eqref{eqn:general_langevin}:
...
In our case, to simulate the dispersion of chromatin only around DSBs, we induce volume exclusion for the DF monomers only and take $\kappa_{ev} = \kappa$:
...
\begin{enumerate}
	\item \textbf{Initialization of the polymer backbone} from a random walk in $\mathbb{R}^3$ with each step is i.i.d. $\mathcal{N}(0,b^2)$.
	...
	\tau = \frac{...}{...}
	$$
	\item \textbf{DSBs Induction and actualization of the total Laplacian matrix}. When the polymer reaches relaxation time, the two cuts are induced between the predefined monomers $A_1$, $A_2$ and $B_1$, $B_2$. Removed bonds are thus cleaned from the Laplacian matrix. Besides, if CLs are removed from the separated monomers and if Volume Exclusion is included, those effects are also added to the Laplacian as in Eq. \ref{eqn:langevin_VE}.
	\item \textbf{Simulation until encounter}. Once the DSBs are considered into the Laplacian matrix, simulation of Eq. \ref{eqn:langevin} continues until an encounter, as defined in Eq. \ref{eqn:encountertime}, happens between any of the cut ends. If no encounters occur before a threshold time $T_{max}$ the simulation is discarded.
...
\subsubsection{Effect of Excluded Volume}
 ...
 Larger values of $\sigma$ make the waiting for looping times last longer (figure \ref{fig:mFET_v_Nc_EV}) and thus make the encounter events rarer, however they do not affect dramatically the repair probability overall (figure \ref{fig:vs_volumeexclusionradius}). 
