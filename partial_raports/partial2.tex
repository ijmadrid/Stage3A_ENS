...
Considering an adaptive $\varepsilon$ we compare the repair probability when CLs are kept or not in the DF as a function of $N_c$.  It can be observed that the removal allows to increase the repair rates, swifting up the probability functions as in Fig. \ref{fig:proba_v_Nc_Nd2_g20} (for $g = 20$) and \ref{fig:proba_v_Nc_Nd2_g4} (for $g = 4$). The other statistical properties measured in both cases can be observed in Fig. \ref{fig:all_v_Nc_Nd2}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../DNA_Religation/projects/RCLPolymer_TwoDomains/figures/proba_v_Nlr_100TAD.png}
	\caption{Probability of repair for $N_{lr} = $ 0,2,4 and 6 long-range connectors, with a 100-monomers juxtaposed unbroken sub-domain, both with $N_c = 12$. Two DSBs were induced at $g = 10$ in the first sub-domain (100 monomers).}
	\label{fig:proba_v_longrange100}
\end{figure}
...
\begin{itemize}
	...
	\item Concerning the volume exclusion, best repair rates are reached for $\kappa_{ve}$ near to, but smaller than, the backbone $\kappa$ and $\sigma$ a bit lesser than $b$. Increasing values of $\sigma$ also lead to shorter MFET.
